...
parse_tool_gcc = '/home/code/some_tool/tool_gcc'
parse_tool_clang = '/home/code/some_tool/tool_clang'
...
GCC = '/home/software/gcc-trunk/bin/gcc'
CLANG = 'clang'
                ...
                if 'Struct>>' in line:
                    ...
                    for detail_info in split_info:
                        if 'Filename>>' in detail_info:
                            continue
                        # if '(unnamed' in detail_info:
                        #     items = detail_info.split(':')[:2]
                        #     k, v = items
                        #     v = v.strip().split()[0]
                        if len(detail_info.split('>>')) > 2:
                            items = detail_info.split('>>')[:2]
                            k, v = items
                            v = v.strip().split()[0]
                        else:
                            k, v = detail_info.split('>>')
                        var_info[k.strip()] = v.strip()
                ...
                if 'Variable>>' in line:
                    ...
                    for detail_info in split_info:
                        ...
                        if len(detail_info.split('>>')) > 2:
                            ...
                            k, v = detail_info.split('>>')
                        var_info[k.strip()] = v.strip()
                    if not var_info['Line'].isdigit() or not var_info['endColumn'].isdigit():
                        continue
                    if ('[' in var_info['Type']) and (']' in var_info['Type']):
                        continue
                ...
                if 'Definition>>' in line:
                    ...
                    for detail_info in split_info:
                        if 'Filename>>' in detail_info:
                            continue
                        k, v = detail_info.split('>>')
                        func_info[k.strip()] = v.strip()
                    func_list.append(func_info)
    except Exception as e:
        print(f'[Prase Error]:[Prog]:{c_file}\n[Exception]:{e}')

    if os.path.exists(log_file):
        os.remove(log_file)

    if (not struct_var_list) and (not var_list) and (not func_list):
        return
    
    return [struct_var_list, var_list, func_list]

def parse_info_clang(c_file, option, tmp_dir):
    log_file = f"{tmp_dir}/{os.path.basename(c_file).replace('.c', '.log')}"
    try:
        ...
        with open(log_file, 'r') as f:
            for line in f:
                ...
                if 'Loop;' in line:
                    ...
                    split_info = line.strip().split(';')
                    for detail_info in split_info:
                        if 'Loop' in detail_info:
                            continue
                        if 'Filename' in detail_info:
                            continue
                        k, v = detail_info.split('>>')
                        loop_info[k.strip()] = v.strip()
    ...
    if (not struct_var_list) and (not var_list) and (not func_list) and (not loop_list) and (not struct_list):
        return
    
    return [struct_var_list, var_list, func_list, loop_list, struct_list]
...
def generate_new(insert_plan, org_file, new_file):
    line_cnt = 0
    new_prog = ''
    with open(org_file, 'r') as f:
        for line in f:
...
def generate_clang_new(insert_plan, org_file, new_file):
    line_cnt = 0
    new_prog = ''
    with open(org_file, 'r') as f:
        for line in f:
...
def duplicate(prog, info, know_bug):
    for bug in know_bug:
        if bug.strip()is None:
            continue
        if bug.lower() in info:
            print(f'[{prog}]: duplicate of {bug.lower()}')
            return True
        

if __name__ == '__main__':   
    info = parse_ast_clang('bug_gcc_performance_bottleneck.c', '', '/home/code/some_tool')
    for _ in info[-2]:
        print(_)
